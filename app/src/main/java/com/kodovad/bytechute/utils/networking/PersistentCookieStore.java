package com.kodovad.bytechute.utils.networking;
...
public class PersistentCookieStore implements CookieStore {
    ...
    public void add(URI uri, HttpCookie cookie) {
        ...
        if (!cookie.hasExpired()) {
            ...
            Objects.requireNonNull(cookies.get(uri.getHost())).put(name, cookie);
        } else {
            if(cookies.containsKey(uri.toString()))
                Objects.requireNonNull(cookies.get(uri.getHost())).remove(name);
        }

        // Save cookie into persistent store
        SharedPreferences.Editor prefsWriter = cookiePrefs.edit();
        prefsWriter.putString(uri.getHost(), TextUtils.join(",", Objects.requireNonNull(cookies.get(uri.getHost())).keySet()));
    ...
    public List<HttpCookie> get(URI uri) {
        ...
        if(cookies.containsKey(uri.getHost()))
            ret.addAll(Objects.requireNonNull(cookies.get(uri.getHost())).values());
        return ret;
    }
    ...
    public boolean remove(URI uri, HttpCookie cookie) {
        ...
        if(cookies.containsKey(uri.getHost()) && Objects.requireNonNull(cookies.get(uri.getHost())).containsKey(name)) {
            Objects.requireNonNull(cookies.get(uri.getHost())).remove(name);
